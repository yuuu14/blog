...
`tmux new -d` create new session without being attached
`tmux list-session` check living sessions
`tmux attach -t NUM`  go back to session NUM


**the PS1 color issues**
- try add `set -g default-terminal "xterm-256color"` to `~/.tmux.conf` and run `tmux source ~/.tmux.conf`
- try using `tmux set -g default-terminal "xterm-256color"` after create new session without being attached `tmux new -d`, then create new session should be fine
- try set `TERM=xterm-256color` in a created tmux session, then create new session should be fine
